...
const DownloadStatement: React.FC = () => {
  ...
  useEffect(() => {
    if (!customerId || !shopId) {
      Alert.alert("Missing parameters", "customerId or shopId is missing.");
      setLoading(false);
      return;
    }
    ...
    const load = async () => {
      ...
      // subscribe to transactions for this customer + shop
      try {
        const q = query(
        ...
        unsubscribeTransactions = onSnapshot(
          ...
          (snapshot) => {
            if (cancelled) return;
            // optional auth check (keeps parity with your existing behaviour)
  ...
  // Build statement HTML - same style as your old function
      ...
      <html>
        ...
        <body>
          <div class="card">
            ...
            <div class="muted" style="margin-top: 16px;">Generated by HisabKitaab</div>
          </div>
        </body>
      </html>
    `;
  ...
  const handleDownloadStatement = async () => {
    ...
    try {
      ...
      // Log the download to owners/{shopId}/invoiceDownloads
      try {
        await addDoc(collection(db, "owners", shopId, "invoiceDownloads"), {
          ...
          source: "DownloadStatementScreen",
        });
      } catch (logErr) {
  ...
  // Download single transaction invoice (re-uses existing invoice html structure)
    ...
    <SafeAreaView edges={["top", "left", "right"]} className="flex-1 bg-white edges">
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.hint}>
          Customer: <Text style={{ fontWeight: "700" }}>{customer?.name || customerId}</Text>
        </Text>
        <Text style={styles.hint}>
          Shop: <Text style={{ fontWeight: "700" }}>{shopInfo?.shopName || shopId}</Text>
        </Text>
